...
- [ ] 在 Kind 集群解决或绕开 Promtail `nofile` 限制并完成日志链路验收。
- [ ] 完成 PVC 后端重启恢复验收。
- [ ] 把最终动态验收摘要写回本计划并将总状态改为完成。
...
- [ ] 动态证明 Pod 重启后 metrics、traces 和 logs 仍可查询；执行 Task 8。
...
- [ ] 在支持 Promtail 正常启动的环境完成 Loki 日志查询；执行 Task 8。
...
- [ ] Promtail 在该 Kind 环境未通过：containerd 未采用 pod 级 `nofile` 设置，历史验证时为 CrashLoopBackOff。
- [ ] Loki 日志查询未通过：Kind 覆盖值禁用了 Promtail，因此 Loki 为空。
...
- [ ] **Step 4：在 Kind 目标环境部署，并启用 Promtail。**
...
- [ ] **Step 5：建立 Kind port-forward 并验证三信号。**
...
- [ ] 动态验收日期和 Kubernetes 类型/版本。
- [ ] Promtail Ready 证据。
- [ ] Loki 返回应用日志证据。
- [ ] 重启前后 metrics、traces、logs 查询摘要。
- [ ] PVC Bound 状态及恢复结论。
- [ ] 最终提交、分支或合并状态。
...
- [ ] Promtail Ready，Loki 可以查询到 Demo 应用日志。
- [ ] Prometheus、Jaeger、Loki 在 Pod 重启后保留并返回既有数据。
- [ ] Task 8 的动态验证命令全部退出 0。
- [ ] 最终验收记录已写回本文件且工作树干净。
